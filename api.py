...
async def health():
    return {"ok": True, "version": "5.23.0", "providers": ["tmdb", "vidsrc", "4khdhub", "hubcloud", "ytmusic", "jiosaavn", "deezer", "piped", "lrclib", "moviebox-api", "netmirror", "vixsrc"]}
...
async def music_yt_play(video_id: str):
    ...
    # 2) Optional yt-dlp only if installed (not required)
    try:
    ...
    if not rid:
        ...
        for st in pi.get("streams") or []:
            if st.get("id"):
                ...
                break
    if not rid:
        return {"ok": True, "creator": "ElitePlex-direct", "ext_captions": [], "subject_id": subjectId}
    try:
        data = await mb_request(
            "GET",
